...
class OutputController(AbstractController, threading.Thread):
    ...
    def loop(self):
        ...
        for output_id in self.output:
            ...
            if (self.output[output_id].output_setup and
                    self.output[output_id].output_on_until < datetime.datetime.now() and
                    self.output[output_id].output_on_duration and
                    not self.output[output_id].output_off_triggered):

                # Use a thread to prevent blocking the loop
                self.output[output_id].output_off_triggered = True
    ...
    def del_output(self, output_id):
        ...
        """
        try:
            self.logger.debug("Output {id} Deleted.".format(
                id=output_id))
    ...
    def output_on_off(self,
        ...
        """
        # Checks to perform before output is to turn on
        if state == 'on':
            ...
            if (output_type == 'on_off' and
                    ...
                    not self.output[output_id].is_on()):
    ...
    def current_amp_load(self):
        ...
        for each_output_id, each_output_amps in self.output_amps.items():
            if self.output[each_output_id].is_on() and each_output_amps:
    ...
    def output_state(self, output_id):
        ...
        :rtype: str
        """
        if output_id not in self.output or not self.output[output_id].output_setup:
            return

        state = self.output[output_id].is_on()
